...
ACCESS_PERM = "euni_phpbb3.access_euni_phpbb3"
...
def activate_forum(request):
    ...
    logger.debug(
        f"Adding phpbb user for user {request.user} with main character {character}"
    )
    result = Phpbb3Manager.add_user(
        Phpbb3Tasks.get_username(request.user),
        request.user.email,
        ["REGISTERED"],
    ...
    # if empty we failed
    if result[0] != "":
        Phpbb3User.objects.update_or_create(
        ...
        logger.error(
            "Unsuccessful attempt to activate forum for user %s" % request.user
        )
        messages.error(
            request, _("An error occurred while processing your forum account.")
        )
...
def reset_forum_password(request):
    ...
    if Phpbb3Tasks.has_account(request.user):
        ...
        result = Phpbb3Manager.update_user_password(
            request.user.euni_phpbb3.username, character.character_id
        )
        # false we failed
        if result != "":
            ...
            credentials = {
                "username": request.user.euni_phpbb3.username,
                "password": result,
            }
            return render(
                request,
    ...
    logger.error(
        "Unsuccessful attempt to reset forum password for user %s" % request.user
    )
...
def set_forum_password(request):
    ...
    if request.method == "POST":
        ...
        if form.is_valid() and Phpbb3Tasks.has_account(request.user):
            ...
            character = request.user.profile.main_character
            result = Phpbb3Manager.update_user_password(
                request.user.euni_phpbb3.username,
                character.character_id,
                password=password,
            )
            if result != "":
                logger.info(
                    "Successfully set forum password for user %s" % request.user
                )
                messages.success(request, _("Set forum password."))
            else:
                logger.error(
                    "Failed to install custom forum password for user %s" % request.user
                )
                messages.error(
                    request, _("An error occurred while processing your forum account.")
                )
