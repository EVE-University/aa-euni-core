...
class Phpbb3Tasks:
    ...
    def delete_user(cls, user, notify_user=False):
        if cls.has_account(user):
            logger.debug(
                f"User {user} has forum account {user.euni_phpbb3.username}. Deleting."
            )
            if Phpbb3Manager.disable_user(user.euni_phpbb3.username):
                user.euni_phpbb3.delete()
                if notify_user:
    ...
    def has_account(user):
        try:
            return user.euni_phpbb3.username != ""
    ...
    def update_groups(self, pk):
        ...
        if Phpbb3Tasks.has_account(user):
            ...
            try:
                Phpbb3Manager.update_groups(user.euni_phpbb3.username, groups)
            except Exception:
                logger.exception(
                    "Phpbb group sync failed for %s, retrying in 10 mins" % user
                )
