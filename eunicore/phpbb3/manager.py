...
from .utf8_sanitizer import utf8_clean_string, utf8_sanitize_string
...
class Phpbb3Manager:
    SQL_ADD_USER = (
        r"INSERT INTO %susers (username, username_clean, "
        r"user_password, user_email, group_id, user_regdate, user_permissions, "
        r"user_sig, user_lang) VALUES (%%s, %%s, %%s, %%s, %%s, %%s, %%s, %%s, 'en')"
        % TABLE_PREFIX
    )

    SQL_DEL_USER = r"DELETE FROM %susers where username_clean = %%s" % TABLE_PREFIX

    SQL_DIS_USER = (
        r"UPDATE %susers SET user_email= %%s, user_password=%%s WHERE username_clean = %%s"
        % TABLE_PREFIX
    )

    SQL_USER_ID_FROM_USERNAME = (
        r"SELECT user_id from %susers WHERE username_clean = %%s" % TABLE_PREFIX
    )

    SQL_ADD_USER_GROUP = (
        r"INSERT INTO %suser_group (group_id, user_id, user_pending) VALUES (%%s, %%s, %%s)"
        % TABLE_PREFIX
    )

    SQL_GET_GROUP_ID = (
        r"SELECT group_id from %sgroups WHERE group_name = %%s" % TABLE_PREFIX
    )

    SQL_ADD_GROUP = (
        r"INSERT INTO %sgroups (group_name,group_desc,group_legend) VALUES (%%s,%%s,0)"
        % TABLE_PREFIX
    )

    SQL_UPDATE_USER_PASSWORD = (
        r"UPDATE %susers SET user_password = %%s WHERE username_clean = %%s"
        % TABLE_PREFIX
    )

    SQL_REMOVE_USER_GROUP = (
        r"DELETE FROM %suser_group WHERE user_id=%%s AND group_id=%%s " % TABLE_PREFIX
    )

    SQL_GET_ALL_GROUPS = r"SELECT group_id, group_name FROM %sgroups" % TABLE_PREFIX

    SQL_GET_USER_GROUPS = (
        r"SELECT %(prefix)sgroups.group_name FROM %(prefix)sgroups , %(prefix)suser_group WHERE "
        r"%(prefix)suser_group.group_id = %(prefix)sgroups.group_id AND user_id=%%s"
        % {"prefix": TABLE_PREFIX}
    )

    SQL_ADD_USER_AVATAR = (
        r"UPDATE %susers SET user_avatar_type=2, user_avatar_width=64, user_avatar_height=64, "
        "user_avatar=%%s WHERE user_id = %%s" % TABLE_PREFIX
    )

    SQL_CLEAR_USER_PERMISSIONS = (
        r"UPDATE %susers SET user_permissions = '' WHERE user_id = %%s" % TABLE_PREFIX
    )

    SQL_DEL_SESSION = (
        r"DELETE FROM %ssessions where session_user_id = %%s" % TABLE_PREFIX
    )

    SQL_DEL_AUTOLOGIN = (
        r"DELETE FROM %ssessions_keys where user_id = %%s" % TABLE_PREFIX
    )
    ...
    def __add_avatar(username, characterid):
        logger.debug(
            f"Adding EVE character id {characterid} portrait as phpbb avater for user {username}"
        )
    ...
    @staticmethod
    def _santitize_username(username):
        return utf8_sanitize_string(username)

    @staticmethod
    def _sanitize_groupname(name):
        return utf8_sanitize_string(name)
    ...
    def __get_user_id(username_clean):
        ...
        row = cursor.fetchone()
        if row is not None:
            logger.debug(f"Got phpbb user id {row[0]} for username {username_clean}")
            return row[0]
        else:
            logger.error(
                "Username %s not found on phpbb. Unable to determine user id."
                % username_clean
            )
            return None
    ...
    def __create_group(groupname):
        ...
        groupname = Phpbb3Manager._sanitize_groupname(groupname)
        cursor = connections["phpbb3"].cursor()
    ...
    def __add_user_to_group(userid, groupid):
        ...
        try:
            ...
            logger.exception(
                f"Unable to add phpbb user id {userid} to group id {groupid}"
            )
            pass
    ...
    def __remove_user_from_group(userid, groupid):
        ...
        try:
            ...
            logger.exception(
                f"Unable to remove phpbb user id {userid} from group id {groupid}"
            )
            pass
    ...
    def check_user(username):
        ...
        username = Phpbb3Manager._santitize_username(username)
        username_clean = Phpbb3Manager._clean_username(username)

        if Phpbb3Manager.__get_user_id(username_clean):
    ...
    def add_user(username, email, groups, characterid):
        logger.debug(
            "Adding phpbb user with username {}, email {}, groups {}, characterid {}".format(
        ...
        logger.debug(
            f"Proceeding to add phpbb user {username} and pwhash starting with {pwhash[0:5]}"
        )
        # check if the username was simply revoked
        if Phpbb3Manager.check_user(username):
            logger.warning(
                "Unable to add phpbb user with username %s - already exists. Updating user instead."
                % username
            )
            Phpbb3Manager.__update_user_info(username, email, pwhash)
        else:
            try:
                cursor.execute(
                    Phpbb3Manager.SQL_ADD_USER,
                    [
                        username,
                        username_clean,
                        pwhash,
                ...
                Phpbb3Manager.update_groups(username, groups)
                Phpbb3Manager.__add_avatar(username, characterid)
                logger.info("Added phpbb user %s" % username)
            except Exception:
                logger.exception("Unable to add phpbb user %s" % username)
                pass
    ...
    def disable_user(username):
        ...
        try:
            pwhash = Phpbb3Manager.__gen_hash(password)
            cursor.execute(
                Phpbb3Manager.SQL_DIS_USER, [revoke_email, pwhash, username_clean]
            )
            ...
            logger.exception(
                "TypeError occured while disabling user %s - failed to disable."
                % username
            )
            return False
    ...
    def delete_user(username):
        ...
        if Phpbb3Manager.check_user(username):
            cursor.execute(Phpbb3Manager.SQL_DEL_USER, [username_clean])
            logger.info("Deleted phpbb user %s" % username)
            return True
        logger.error(
            "Unable to delete phpbb user %s - user not found on phpbb." % username
        )
        return False
    ...
    def update_groups(username, groups):
        ...
        if userid is not None:
            ...
            logger.info(
                f"Updating phpbb user {username} groups - adding {addgroups}, removing {remgroups}"
            )
            for g in addgroups:
    ...
    def remove_group(username, group):
        ...
        username = Phpbb3Manager._santitize_username(username)
        username_clean = Phpbb3Manager._clean_username(username)
        group = Phpbb3Manager._sanitize_groupname(group)
        userid = Phpbb3Manager.__get_user_id(username_clean)
        if userid is not None:
            ...
            if userid:
                if groupid:
                    try:
                        cursor.execute(
                            Phpbb3Manager.SQL_REMOVE_USER_GROUP, [userid, groupid]
                        )
    ...
    def update_user_password(username, characterid, password=None):
        ...
        if Phpbb3Manager.check_user(username):
            ...
            cursor.execute(
                Phpbb3Manager.SQL_UPDATE_USER_PASSWORD, [pwhash, username_clean]
            )
            Phpbb3Manager.__add_avatar(username, characterid)
            logger.info("Updated phpbb user %s password." % username)
            return password
        logger.error(
            "Unable to update phpbb user %s password - user not found on phpbb."
            % username
        )
        return ""
    ...
    def __update_user_info(username, email, password):
        ...
        try:
            cursor.execute(
                Phpbb3Manager.SQL_DIS_USER, [email, password, username_clean]
            )
