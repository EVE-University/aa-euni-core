...
MODULE_PATH = "eunicore.phpbb3"
DEFAULT_AUTH_GROUP = "Member"


def add_permissions():
    permission = Permission.objects.get(codename="access_euni_phpbb3")
...
class Phpbb3HooksTestCase(TestCase):
    ...
    def test_update_groups(self):
        ...
        with mock.patch(MODULE_PATH + ".tasks.Phpbb3Manager") as manager:
            ...
            self.assertEqual(user_id, member.euni_phpbb3.username)
    ...
    def test_validate_user(self, manager):
        ...
        self.assertTrue(member.euni_phpbb3)
        ...
        with self.assertRaises(ObjectDoesNotExist):
            User.objects.get(username=self.none_user).euni_phpbb3
    ...
    def test_delete_user(self, manager):
        ...
        self.assertTrue(manager.disable_user.called)
        with self.assertRaises(ObjectDoesNotExist):
            User.objects.get(username=self.member).euni_phpbb3
    ...
    def test_render_services_ctrl(self):
        ...
        self.assertIn(urls.reverse("euni_phpbb3:deactivate"), response)
        self.assertIn(urls.reverse("euni_phpbb3:reset_password"), response)
        self.assertIn(urls.reverse("euni_phpbb3:set_password"), response)

        # Test register becomes available
        member.euni_phpbb3.delete()
        member = User.objects.get(username=self.member)
        request.user = member
        response = service.render_services_ctrl(request)
        self.assertIn(urls.reverse("euni_phpbb3:activate"), response)
...
class Phpbb3ViewsTestCase(TestCase):
    ...
    def test_activate(self, manager, tasks_manager):
        ...
        response = self.client.get(urls.reverse("euni_phpbb3:activate"))
    ...
    def test_deactivate(self, manager):
        ...
        response = self.client.get(urls.reverse("euni_phpbb3:deactivate"))
        ...
        with self.assertRaises(ObjectDoesNotExist):
            User.objects.get(pk=self.member.pk).euni_phpbb3
    ...
    def test_set_password(self, manager):
        ...
        response = self.client.post(
            urls.reverse("euni_phpbb3:set_password"), data={"password": "1234asdf"}
        )
    ...
    def test_reset_password(self, manager):
        ...
        response = self.client.get(urls.reverse("euni_phpbb3:reset_password"))
...
class UTF8SanitizerTestCase(TestCase):
    def test_clean_case_folding(self):
        input = "MiXed CasE"
        expected = "mixed case"

        self.assertEqual(utf8_clean_string(input), expected)

    def test_clean_excess_spaces(self):
        input = "  too  many     spaces  can not  handle      "
        expected = "too many spaces can not handle"

        self.assertEqual(utf8_clean_string(input), expected)

    def test_clean_confusables(self):
        input = b"we\xC2\xA1rd\xE1\x9A\x80ch\xCE\xB1r\xC2\xADacters".decode("utf8")
        expected = "weird characters"

        self.assertEqual(utf8_clean_string(input), expected)

    def test_sanitize_excess_spaces(self):
        input = "  too  many     spaces  can not  handle      "
        expected = "too many spaces can not handle"

        self.assertEqual(utf8_sanitize_string(input), expected)

    def test_sanitize_confusables(self):
        input = b"we\xC2\xA1rd\xE1\x9A\x80ch\xCE\xB1r\xC2\xADacters".decode("utf8")
        expected = "weird characters"
